...
selector_func <- function(train_s, data, Q.SL.library, d.SL.library, g.SL.library, pRCT = pRCT, family, family_nco, fluctuation = "logistic", NCO=NULL, Delta=NULL, Delta_NCO = NULL, adjustnco=adjustnco, target.gwt=target.gwt, Q.discreteSL=Q.discreteSL, d.discreteSL=d.discreteSL, g.discreteSL=g.discreteSL){
  ...
  if(any(train_s$S==0)){
    ...
    #-------------------------------------------------
    if(target.gwt){
      wt_s <- as.numeric(XS$A==0 & XS$S==1 & XS$Delta == 1)/.bound((gSHat1W*gHat0W*dHat1W$A0),nrow(train_s))
      ...
      wt_s <- rep(1, nrow(XS))
      H.SAW<- as.numeric(XS$A==0 & XS$S==1 & XS$Delta == 1)/.bound((gSHat1W*gHat0W*dHat1W$A0),nrow(train_s))

      # also want to evaluate the clever covariates at S=1 and A=0 for all subjects
      H.S0W<- 1/.bound((gSHat1W*gHat0W*dHat1W$A0),nrow(train_s))
    }
...
IORD_cvtmle_sims <- function(data, study, covariates, treatment_var, treatment, outcome, NCO=NULL, Delta=NULL, Delta_NCO=NULL, pRCT, V, Q.SL.library, d.SL.library, g.SL.library, Q.discreteSL, d.discreteSL, g.discreteSL, family, family_nco, fluctuation = "logistic", comparisons = list(c(0,1)), adjustnco = TRUE, target.gwt = TRUE){
  ...
  for(v in 1:length(folds)){
    ...
    for(s in 1:length(comparisons)){
      ...
      if(s==1){
        ...
        #------------------------------------------
        if(fluctuation == "logistic"){
          logitUpdate<- glm(selector[[v]][[s]]$Y[which(train_s$Delta==1)] ~ -1 + offset(qlogis(selector[[v]][[s]]$QbarSAW[which(train_s$Delta==1)])) +  selector[[v]][[s]]$H.SAW[which(train_s$Delta==1)], family='binomial', weights = selector[[v]][[s]]$wt_s[which(train_s$Delta==1)])
      ...
      if(s>1){
        EICpsipound[which(data$v!=v & data$S %in% comparisons[[s]]),(length(comparisons)*(v-1)+s)] <- (selector[[v]][[s]]$wt_s*(selector[[v]][[s]]$H.SAW)*(trainsY - QbarSAW.star) + QbarS0W.star - mean(QbarS0W.star) + selector[[v]][[s]]$wt*(selector[[v]][[s]]$H.AW0)*(trainsY - QbarAW.star) - Qbar0W.star + mean(Qbar0W.star))/(length(which(train$S %in% comparisons[[s]]))/nrow(data))
      }

      bias[[v]][s] <- mean(QbarS0W.star) - mean(Qbar0W.star)
  ...
  if(is.null(NCO)==FALSE){
    ...
    #ztilde_poundplusphi <- c(psipoundplusphivec, psivec)
    #ztilde_phi <- c(phivec, psivec)
  }
  ...
  for(b in 1:1000){
    ...
    for(v in 1:V){
      ...
      #sample_psi_pstarnv[[b]][v,] <- psisamp[b,((length(comparisons)*(v-1)+1):(length(comparisons)*(v)))] + limitdist$psi_s
    }
  }

  #check <- matrix(NA, 1000,2)
  #for(b in 1:1000){
  #  check[b,1] <- mean(sample_psi_pstarnv[[b]][,1])
  #  check[b,2] <- mean(sample_psi_pstarnv[[b]][,2])
  #}
  #summary(check)

  #add back estimated \Psi_s(P_0)
  ...
  if(any(unlist(proportionselected$b2v)!=1)){
    ...
    #results$CI$b2v <- quantile(psi_pstarnv_b2v, probs = c(0.025,0.5,0.975))
  ...
  if(is.null(NCO)==FALSE){
    ...
    if(any(unlist(proportionselected$ncobias)!=1)){
      ...
      #results$CI$ncobias <- quantile(psi_pstarnv_nco, probs = c(0.025,0.5,0.975))
    ...
    if(any(unlist(proportionselected$ncoonly)!=1)){
      ...
      #results$CI$ncoonly <- quantile(psi_pstarnv_ncoonly, probs = c(0.025,0.5,0.975))
